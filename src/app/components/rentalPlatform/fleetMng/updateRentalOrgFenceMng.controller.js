...
(function() {
  ...
  angular
    .module('GPSCloud')
    .controller('updateRentalOrgFenceController', updateRentalOrgFenceController);

  /** @ngInject */
  function updateRentalOrgFenceController($rootScope,$window,$scope,$stateParams ,$timeout,$http,$confirm,$uibModal,$location,rentalService,treeFactory,serviceResource,RENTAL_ORG_FENCE_URL,AMAP_GEO_CODER_URL, Notification) {
    var vm = this;
    vm.rentalOrgFence={};
    vm.status={};
   ...
   // vm.radius = 100; //设置的半径,默认100米
    ...
    vm.ok = function () {
      vm.rentalOrgFence.location=vm.selectAddress;
      vm.rentalOrgFence.radius=vm.radius;
      vm.rentalOrgFence.org=vm.org;
      vm.rentalOrgFence.status=vm.status.value;


      var rspdata = serviceResource.restUpdateRequest(RENTAL_ORG_FENCE_URL,vm.rentalOrgFence);
    //  vm.rentalCustomer.org=vm.org;
      rspdata.then(function (data) {
        Notification.success("更新围栏成功!");
        $location.path(path);
